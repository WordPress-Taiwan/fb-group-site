...
if( !function_exists("theme_styles") ) {  
    function theme_styles() { 
        ...
        // 天氣icon font
        wp_register_style( 'weather-icons', get_template_directory_uri() . '/library/css/weather-icons.min.css', array(), '1.0', 'all' );
...
if( !function_exists( "theme_js" ) ) {  
  function theme_js(){
    ...
    wp_register_script( 'modern_biz', 
      get_template_directory_uri() . '/library/js/modern-business.js', 
      array('jquery'), 
      '1.2' );
...
function KH_temp() {
  ...
  if(!empty($location)){
      foreach($xml->channel->item as $item){
          ...
          $current = $current[0];
          /*
          echo 'city: '. $location[0]['city'].'<br>';
          echo 'region: '. $location[0]['region'].'<br>';
          echo 'date:' .$current['date'].'<br>';
          echo 'temp:' .$current['temp'].'&deg;F <br>';
          echo 'code:' .$current['code'].'<br>';
          echo 'text:' .$current['text'].'<br>';
          echo "{$forecast[0]['day']} - {$forecast[0]['text']}. High: {$forecast[0]['high']} Low: {$forecast[0]['low']}".'<br>';
          echo "{$forecast[1]['day']} - {$forecast[1]['text']}. High: {$forecast[1]['high']} Low: {$forecast[1]['low']}".'<br>';
          */
          $f_temp = intval($current['temp']);
          $c_temp = intval(($f_temp-32)*5/9);
          $cur_time = date('Y/m/d H:i A', strtotime(current_time( 'mysql' )));
          $wea_icon = get_bloginfo('template_url').'/images/weather/'.$current['code'].'.png';
          $weather_output = '<img class="wea_icon" src="'.$wea_icon.'"> <img src="'.get_bloginfo('template_url').'/images/apart-line.png" style="margin:0 5px;"> <span class="nav_time">'.$cur_time .'</span> <span class="nav_temp">'. $c_temp . '&deg;C / ' . $f_temp . '&deg;F</span>';
      }
  }else{
      $weather_output = '';
  }
