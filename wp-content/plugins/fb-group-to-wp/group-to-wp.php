...
class WeDevs_FB_Group_To_WP {
    ...
    public function register_post_type() {
        ...
        $args = array(
            ...
            'labels'              => $labels,
            'supports'            => array( 'title', 'editor', 'post-formats', 'comments','author' ),
        ...
        if ( $group_posts ) {
            foreach ($group_posts as $fb_post) {
                ...
                if ( $post_id ) {
                    ...
                    if ( property_exists( $fb_post, 'id' ) ) {
                        $option       = $this->get_settings();
                        $access_token = $option['app_id'] . '|' . $option['app_secret'];
                        $fbc_url = 'https://graph.facebook.com/'.$fb_post->id.'/comments/?access_token='.$access_token;
                        $json_posts  = $this->fetch_stream( $fbc_url );
                        $decoded     = json_decode( $json_posts );
                        $comments = $decoded->data;
                        $comment_count = $this->insert_comments( $post_id, $comments);
                    }
